...
struct DeviceMotionRange motion_range_cache[MOTION_RANGE_CACHE_MAX];
int motion_range_cache_items = 0;

static bool init_done = false;
static bool key_state[OURKEY_COUNT] = {false};
static _getAxisValue_sig getAxisValue = nullptr;
...
static void init() {
    ...
    if ((lib_android = dlopen("libandroid.so", 0))) {
        ...
        LOGD("AMotionEvent_getAxisValue() address is %p", getAxisValue);
    } else {
        LOGD("Failed to open libandroid.so.");
    }
}

static int translate_keycode(int code) {
    switch (code) {
...
static void report_key_state(int keyCode, bool state,
                              CookedEventCallback callback) {
    bool wentDown = !key_state[keyCode] && state;
    bool wentUp = key_state[keyCode] && !state;
...
static void report_key_states_from_axes(float x, float y,
                                         CookedEventCallback callback) {
    report_key_state(OURKEY_LEFT, x < -0.5f, callback);
    report_key_state(OURKEY_RIGHT, x > 0.5f, callback);
    report_key_state(OURKEY_UP, y < -0.5f, callback);
    report_key_state(OURKEY_DOWN, y > 0.5f, callback);
}

static bool process_keys(bool isJoy, AInputEvent *event,
                          CookedEventCallback callback) {
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY) {
        int action = AKeyEvent_getAction(event);
        int code = translate_keycode(AKeyEvent_getKeyCode(event));
        bool handled = code >= 0;
        if (code >= 0 && action == AKEY_EVENT_ACTION_DOWN) {
            report_key_state(code, true, callback);
        } else if (code >= 0 && action == AKEY_EVENT_ACTION_UP) {
            report_key_state(code, false, callback);
        }
        return handled;
    } else if (isJoy) {
        ...
        if (getAxisValue) {
            ...
            x += getAxisValue(event, AXIS_HAT_X, 0);
            y += getAxisValue(event, AXIS_HAT_Y, 0);
            x = Clamp(x, -1.0f, 1.0f);
            y = Clamp(y, -1.0f, 1.0f);
        }
        report_key_states_from_axes(x, y, callback);
        return true;
    }
    return false;
}

static void look_up_motion_range(int deviceId, int source, float *outMinX,
                                  float *outMaxX, float *outMinY,
                                  float *outMaxY) {
    int i;
    for (i = 0; i < motion_range_cache_items; i++) {
        DeviceMotionRange *item = &motion_range_cache[i];
    ...
    if (motion_range_cache_items >= MOTION_RANGE_CACHE_MAX) {
        ...
        newItem = &motion_range_cache[i];
    } else {
        // create a new entry
        newItem = &motion_range_cache[motion_range_cache_items++];
    }
...
static bool CookEvent_Joy(AInputEvent *event, CookedEventCallback callback) {
    ...
    process_keys(true, event, callback);
    return callback(&ev);
}
...
static bool CookEvent_Motion(AInputEvent *event, CookedEventCallback callback) {
    ...
    if (ev.motionIsOnScreen) {
        ...
        look_up_motion_range((int)AInputEvent_getDeviceId(event),
...
bool CookEvent(AInputEvent *event, CookedEventCallback callback) {
    ...
    if (isJoy) {
        ...
        bool handled = process_keys(false, event, callback);
