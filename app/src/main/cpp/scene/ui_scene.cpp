...
UiWidget *UiScene::NewWidget() {
    ...
    auto *widget = new UiWidget(mWidgetCount);
...
UiWidget *UiScene::GetWidgetById(int id) {
    return (id < 0 || id >= mWidgetCount) ? nullptr : mWidgets[id];
}
...
void UiScene::OnKeyDown(int ourKeyCode) {
    ...
    if (navDir >= 0) {
        if (mFocusWidget < 0) {
            ...
            UiWidget *destWidget =
                    w ? GetWidgetById(destId = w->GetNav(navDir)) : nullptr;
...
static void apply_transition(int trans, float f, float *x, float *y, float *w,
...
void UiWidget::Render(TrivialShader *trivialShader, TextRenderer *textRenderer,
    ...
    apply_transition(mTransition, transitionFactor, &x, &y, &w, &h, &fontScale);
