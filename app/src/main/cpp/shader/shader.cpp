...
#include "../common.hpp"
#include "../utils/indexbuf.h"
#include "../utils/vertexbuf.h"

Shader::Shader() {
    mVertShaderH = mFragShaderH = mProgramH = 0;
    mMVPMatrixLoc = -1;
    mPositionAttribLoc = -1;
    mPreparedVertexBuf = nullptr;
}
...
void Shader::PushMVPMatrix(glm::mat4 *mat) const {
    MY_ASSERT(mMVPMatrixLoc >= 0);
    glUniformMatrix4fv(mMVPMatrixLoc, 1, GL_FALSE, glm::value_ptr(*mat));
}

// To be called by child classes only.
void Shader::PushPositions(int vbo_offset, int stride) const {
...
TrivialShader::~TrivialShader() = default;
...
int TrivialShader::GetColorAttribLoc() const { return mColorLoc; }
