...
#include "view/input_util.h"
#include "view/joystick-support.h"
#include "scene/scene_manager.h"
#include "scene/welcome_scene.h"
...
void handle_cmd_proxy(struct android_app *app, int32_t cmd) {
    auto *engine = (NativeEngine *) app->userData;
    engine->HandleCommand(cmd);
}

int handle_input_proxy(struct android_app *app, AInputEvent *event) {
    auto *engine = (NativeEngine *) app->userData;
...
void NativeEngine::GameLoop() {
    mApp->userData = this;
    mApp->onAppCmd = handle_cmd_proxy;
    mApp->onInputEvent = handle_input_proxy;
...
//        // If not animating, block until we get an event; if animating, don't block.
        while ((ALooper_pollAll(IsAnimating() ? 0 : -1, nullptr, &events,
                                (void **) &source)) >= 0) {
            // process event
            if (source != nullptr) {
...
JNIEnv *NativeEngine::GetJniEnv() {
    if (!mJniEnv) {
        ...
        if (0 != mApp->activity->vm->AttachCurrentThread(&mJniEnv, nullptr)) {
            LOGE("*** FATAL ERROR: Failed to attach thread to JNI.");
            ABORT_GAME;
        }
        MY_ASSERT(mJniEnv != nullptr);
...
bool cooked_event_callback(struct CookedEvent *event) {
    SceneManager *mgr = SceneManager::GetInstance();
    PointerCoords pointerCoords{};
    memset(&pointerCoords, 0, sizeof(pointerCoords));
    pointerCoords.x = event->motionX;
    pointerCoords.y = event->motionY;
    pointerCoords.minX = event->motionMinX;
    pointerCoords.maxX = event->motionMaxX;
    pointerCoords.minY = event->motionMinY;
    pointerCoords.maxY = event->motionMaxY;
    pointerCoords.isScreen = event->motionIsOnScreen;
    ...
    switch (event->type) {
        ...
        case COOKED_EVENT_TYPE_POINTER_DOWN:
            mgr->OnPointerDown(event->motionPointerId, &pointerCoords);
            return true;
        case COOKED_EVENT_TYPE_POINTER_UP:
            mgr->OnPointerUp(event->motionPointerId, &pointerCoords);
            return true;
        case COOKED_EVENT_TYPE_POINTER_MOVE:
            mgr->OnPointerMove(event->motionPointerId, &pointerCoords);
            return true;
...
bool NativeEngine::PrepareToRender() {
    if (mEglDisplay == EGL_NO_DISPLAY || mEglSurface == EGL_NO_SURFACE ||
        ...
        LOGD("NativeEngine: binding surface and context (display %p, surface %p, "
                "context %p)",
...
bool NativeEngine::InitSurface() {
    ...
    // create EGL surface
    mEglSurface =
            eglCreateWindowSurface(mEglDisplay, mEglConfig, mApp->window, nullptr);
    if (mEglSurface == EGL_NO_SURFACE) {
...
bool NativeEngine::HandleInput(AInputEvent *event) {
    return CookEvent(event, cooked_event_callback);
}
